...
def finetune(chkp,inp_args):
    train_args = chkp['args']
    # wandb.init()
    if inp_args.train_combined:
        dataset = MyDataset_loss_CH(inp_args.data,inp_args.data_ext,inp_args.excl_participant,inp_args.excl_class)
        dataset_size = len(dataset)
        indices = list(range(dataset_size))
        split = int(np.floor((1-inp_args.train_test_split) * dataset_size))
        np.random.shuffle(indices)
        train_indices, val_indices = indices[:split], indices[split:]
        train_sampler = SubsetRandomSampler(train_indices)
        valid_sampler = SubsetRandomSampler(val_indices)
        train_loader = DataLoader(dataset, batch_size=inp_args.batch_size,num_workers = 6,pin_memory = True,sampler=train_sampler)
        test_loader = DataLoader(dataset, batch_size=inp_args.batch_size,num_workers = 6,pin_memory = True,sampler=valid_sampler)
    else:
        dataset_train = MyDataset_loss_CH(inp_args.data,inp_args.data_ext,inp_args.excl_participant,inp_args.excl_class)
        dataset_test = MyDataset_loss_CH(inp_args.data,inp_args.data_ext,inp_args.incl_participant,inp_args.excl_class)
        train_loader = DataLoader(dataset_train, batch_size=inp_args.batch_size,num_workers = 6,pin_memory = True,shuffle = True)
        test_loader = DataLoader(dataset_test, batch_size=inp_args.batch_size,num_workers = 6,pin_memory = True)
    ...
    net = ML_training.SSL_training.pre_train_utils.match_wts(net, chkp['model'], inp_args)
    # optim_fn = optim.SGD(net.parameters(), lr=0.1, momentum=0.9)
    optim_fn = optim.Adam(net.parameters(), lr=inp_args.lr)
    ...
    accuracy = 0
    for epoch in range(inp_args.num_epochs):
        print("Epoch: ", epoch+1)
        t_0 = time.time()
        train_loss_epoch, preds_out, labels_out = train(net, train_loader, crit, optim_fn,scaler, inp_args)
        accuracy_tr = np.float16(accuracy_score(labels_out, preds_out))
        print("Train Accuracy: ",np.float16(accuracy_tr*100),"%")
        test_loss_epoch, preds_out, labels_out = val(net, test_loader,crit,inp_args)
        ...
        if accuracy_itr > accuracy and accuracy_itr>inp_args.model_save_th:
            PATH_model = inp_args.model_save_dir+train_args.model+'_'+str(accuracy_itr*100)+'_'+str(epoch)+'.pth'
            ...
            ML_training.vanilla_training.helper.pkl_dump(inp_args.model_save_dir+inp_args.model+'_ft_'+str(accuracy_itr*100)+'_'+str(epoch)+'_preds.pkl',pred_dict)
            accuracy = accuracy_itr
        sched.step(test_loss_epoch)
        if optim_fn.param_groups[0]['lr'] < inp_args.convergence_lr:
            break

...
class MyDataset_loss_CH(data.Dataset):
        ...
        for i, fname in enumerate(file_path):
            ...
            if participant not in exclusion_list_participant:
                if class_name not in exclusion_list_class:
                    self.data_path.append((fname,int(GESTURE_KEYS[class_name])))
    ...
    def __getitem__(self, index):
        ...
        X, num_nodes = pkl_load(X_path)
        X.edge_index = X.edge_index.T
        return (X,num_nodes), y
    
    
class MyDataset_MLP(data.Dataset):
    def __init__(self, path, ext, exclusion_list_participant, exclusion_list_class):
        file_path = glob.glob(path + r'\**\*'+ext, recursive=True)
        self.data_path = []
        for i, fname in enumerate(file_path):
            ...
            if class_name not in exclusion_list_class and participant not in exclusion_list_participant:
    ...
    def __getitem__(self, index):
        ...
        return (X.x.flatten(),num_nodes), y

if __name__ == "__main__":
    exclusion_list_participant = []
    exclusion_list_class = []
    dataset = MyDataset_loss_Triplet("./data",".pkl",exclusion_list_participant,exclusion_list_class)
